...
fun MenuTopBar() {
    ...
    Box(
        modifier = Modifier
            .fillMaxWidth()
            .heightIn(min = 70.dp, max = if (isTablet) 96.dp else 86.dp)
    ) {
        TopAppBar(
            title = {
                Text(
...
fun ContentScreen(
    ...
    val gridRows = collapsedCategories.chunked(columns)
        ...
        items(gridRows) { row ->
            Row(
                ...
                row.forEach { categoria ->
                    CategoryCardExpandable(
                        categoria = categoria,
                        cardWidth = if (isTablet) 220.dp else 160.dp,
                        navController = navController,
                        isExpanded = false,
                        onToggleExpand = { expanded ->
...
fun CategoryCardExpandable(
    ...
    val coverHeight = if (isExpanded) {
        if (isTablet) 260.dp else 200.dp
    } else {
        if (isTablet) 180.dp else 140.dp
    }
...
fun ItemCardSmall(item: Historia, onClick: () -> Unit = {}) {
    ...
    val cardHeight = if (isTablet) 240.dp else 180.dp
